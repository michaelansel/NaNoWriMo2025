...
- Reviewer agents (`pm`, `architect`, `ceo`) are read-only: `tools: Read, Grep, Glob`, a `maxTurns`, and a `description` that says when to use them proactively. They answer in a fixed order with the verdict first. Never give an agent write tools.
