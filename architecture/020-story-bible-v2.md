...
These notes refine the decision above as it was built. They do not change it.

1. **Prompt rendering lives in `llm/`.** `render_prompt`, `prompt_schema` and `prompt_hash` are in `nanoif/llm/prompts.py`, and the editors and the Bible share them. `bible/` imports `twee/`, `llm/` and `check/`. It never imports `review/`, so imports stay `twee <- bible <- review`.
2. **There is one prose rule.** `nanoif.twee.prose.is_infra` is the only test for "not story prose", and `bible.source` and `review.units` both call it. Under it, a passage is not prose if it is `StoryData`, `StoryTitle`, `StoryStyles`, `PathIdDisplay` or `PathIdLookup`, or has one of the tags `script`, `stylesheet`, `footer`, `header` or `startup`. `Start` is prose. `Start.twee` is a tooling-owned file (see `check.structure.INFRA_FILES`, which exempts it from prose file naming), but its passages are the story's opening, and both the Bible and the editors read them. The two lists answer different questions and stay separate. `INFRA_FILES` says who owns a file and how it is named. `is_infra` says whether a passage is prose.
3. **Pins and retrieval.** A pinned fact is exempt only from the cap of 40 facts per unit. Retrieval still leaves a pin out, and counts it, when its source passage is the one under review or has changed since extraction. AC-story-bible-28 takes precedence over the "always" in AC-story-bible-21, because checking a passage against a quote from its own earlier text produces exactly the false contradiction that AC-28 prevents. A stale pin comes back once its passage is re-extracted. The pack's limit of 6 facts per entity applies to pins too. Pins sort first, so a pin is dropped from the pack only when one entity has more than 6.
4. **The PR dry run.** `/extract-story-bible` runs the default branch's tooling. Only `src/`, `story-overrides.txt` and `ai/story-bible-cache.json` come from `refs/pull/<N>/merge`, so no pull-request code runs with the job's token. Its concurrency group is `ai-cmd-pr-<N>-bible`, so it neither cancels nor is cancelled by `/check-continuity` on the same PR. Every failure after the runner check ends in the `<!-- nano:bible -->` "did not run" comment with a reason. That includes a merge ref that cannot be fetched, for example when the PR conflicts with `main`. A run of this workflow does not show up in the PR's checks, so the comment is the only signal the writer gets.
5. **Serializing `main`.** `bible-extract` has no job-level concurrency. It runs inside the workflow-level `pages` group, which serializes runs on `main` and never cancels one in progress. A job-level group with the same name would wait on its own run and deadlock. While an extraction runs (up to 45 min), later pushes wait. GitHub keeps only the newest pending run, so an intermediate run may be cancelled while it waits, and the next incremental run, which starts from `main` HEAD, covers it. If the exe runner disappears between the probe and the job's start, the job stays queued and holds `pages` (queue time does not count toward `timeout-minutes`) until someone cancels it. That run shows as queued in Actions, and the site stays on its last deploy. `deploy` fetches the `bible-cache` artifact with `pattern:`. When the artifact is absent, this downloads nothing instead of failing, and the step summary names the cache that the page was rendered from.
6. **Aliases merge only at assemble time.** `alias:` lines join entities in `assemble` and never in the cache, so deleting a line splits the entities on the next build without an extraction. Extraction does use the alias lines: it adds them to the extract roster, and resolve routes new names to the canonical entity. It never absorbs an existing cache entity, and model merges only attach new clusters. So `merged_into` is always `null` and the retire reason `merged` is unused. Both stay in the `story_bible_cache` schema as reserved values, because changing the cache contract costs more than keeping them. Reconcile runs per cache entity, so it does not find duplicates or conflicts between two entities that only an `alias:` line joins. The canon pack still shows them to the Continuity Editor as one entity.
