...
- Modify `extract_facts_from_passage()` to parse entity structure
- Add `normalize_entity_names()` function for title/possessive handling
- Update cache structure to support entities
...
```json
{
  "extraction_version": "2.0-entity-first",
...
### Alternative 1: Fact-First with Better Prompts
...
### Alternative 2: Entity Extraction as Separate Pass (Post-Summarization)
...
### Alternative 3: Manual Entity Registry
...
### Alternative 4: Full Cache Rewrite (No Backward Compatibility)
...
### Phase 1: Entity Extractor (Week 1)

**Developer Tasks**:
1. Update `EXTRACTION_PROMPT` in `story_bible_extractor.py` to entity-first template
2. Modify `extract_facts_from_passage()` to parse entity structures
3. Add entity normalization functions (title preservation, possessive stripping)
4. Update cache writes to include `entities` section alongside `facts`
5. Test extraction on known missing entities (Marcie, Miss Rosie, Josie)
6. Verify 100% detection on sample passages
...
- Entity mentions cite correct passages and quotes
