...
- Sticky comments are found by HTML marker (`<!-- nano:build -->`, `<!-- nano:continuity -->`, `<!-- nano:if-editor -->`) and edited in place; one per check type per PR.
- Concurrency: `ci-pr-${{ github.event.number }}` and `ai-cmd-pr-N`, cancel-in-progress on PRs; never on main.
- Pin tool versions in `env:` (tweego, dotgraph) and actions by major tag. Python 3.12 with pip cache; `pip install -e ".[dev]"`, never rely on the runner image.
